...
async function getAccessToken() {
    ...
    connectionSettings = await fetch('https://' + hostname + '/api/v2/connection?include_secrets=true&connector_names=google-sheet', {
        headers: {
...
export async function isGoogleSheetsConnected() {
    try {
...
export async function exportToSheet(spreadsheetId, sheetName, data) {
    const sheets = await getGoogleSheetsClient();
...
export async function readFromSheet(spreadsheetId, range) {
    const sheets = await getGoogleSheetsClient();
...
export async function exportInvoicesToSheet(invoices, spreadsheetId) {
...
export async function exportExpensesToSheet(expenses, spreadsheetId) {
    let sheetId = spreadsheetId;
    if (!sheetId) {
...
export async function exportJournalEntriesToSheet(entries, spreadsheetId) {
    let sheetId = spreadsheetId;
    if (!sheetId) {
...
export async function exportChartOfAccountsToSheet(accounts, spreadsheetId) {
    let sheetId = spreadsheetId;
    if (!sheetId) {
...
export async function exportCustomDataToSheet(title, sheets) {
...
export async function importInvoicesFromSheet(spreadsheetId) {
    const sheets = await getGoogleSheetsClient();
...
export async function importExpensesFromSheet(spreadsheetId) {
    const sheets = await getGoogleSheetsClient();
    // Get all sheets in the spreadsheet
    const response = await sheets.spreadsheets.get({
        spreadsheetId,

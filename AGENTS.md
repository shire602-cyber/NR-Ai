...
- `tests/` contains Vitest unit tests plus the authenticated endpoint smoke script.
...
- Vitest uses `tests/setup.ts`, which sets `NODE_ENV=test`, `PORT=5001`, and a test `DATABASE_URL`.
- Vitest and ESLint intentionally exclude `.claude/` and `.claire/` worktrees. Do not use those

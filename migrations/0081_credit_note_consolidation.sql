-- A-B11/A-B15: make invoice-embedded credit notes canonical.
--
-- Ordering is load-bearing: copy standalone credit_notes into invoices before
-- removing standalone VAT-return subtraction in app code. After this migration,
-- VAT sees credit notes through invoice_lines exactly once.
...
-- Preserve the posted journal entries from the standalone lifecycle but point
-- source/source_id at the canonical invoice credit note for future idempotency.
...
-- If an issued standalone credit note was never posted to GL, post a balanced
-- reversal when the company has the required default accounts and at least one
-- user to satisfy journal_entries.created_by. This fixes the GL/VAT mismatch
-- without touching rows that already had a posted source entry.
WITH candidates AS (
  SELECT

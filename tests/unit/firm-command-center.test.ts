...
describe("calculateHealthScore — boundary cases", () => {
  ...
  it("treats submitted VAT identically to filed", () => {
    expect(calculateHealthScore(baseInputs({ vatStatus: "submitted" })).score).toBe(100);
  });
...
describe("generateAlertsForClient — alert types", () => {
  ...
  it("emits an info incomplete_onboarding alert", () => {
    const alerts = generateAlertsForClient(baseSnapshot({ onboardingCompleted: false }), FIXED_NOW);
  ...
  it("emits document_missing info <5 docs and warning ≥5", () => {
    const small = generateAlertsForClient(baseSnapshot({ missingDocuments: 2 }), FIXED_NOW);
...
describe("calculatePeriodComparison", () => {
  ...
  it("computes correct percentage deltas", () => {
    const c = calculatePeriodComparison(range, { revenue: 110, receipts: 50, invoices: 10 }, prev, {
      revenue: 100,
      receipts: 25,
      invoices: 10,
    });
  ...
  it("handles zero previous period without dividing by zero", () => {
    const c = calculatePeriodComparison(range, { revenue: 50, receipts: 0, invoices: 0 }, prev, {
      revenue: 0,
      receipts: 0,
      invoices: 0,
    });
  ...
  it("reports negative deltas when current < previous", () => {
    const c = calculatePeriodComparison(range, { revenue: 50, receipts: 10, invoices: 5 }, prev, {
      revenue: 100,
      receipts: 20,
      invoices: 5,
    });
  ...
  it("rounds to one decimal", () => {
    const c = calculatePeriodComparison(range, { revenue: 103, receipts: 0, invoices: 0 }, prev, {
      revenue: 97,
      receipts: 0,
      invoices: 0,
    });

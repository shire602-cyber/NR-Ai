...
describe('e-invoicing validation gate', () => {
  ...
  it('requires a valid supplier TRN', () => {
    ...
    const badFormat = validateForEInvoicing(makeInvoice(), lines, { ...company, trnVatNumber: '12345' });
  ...
  it('rejects buyer TRNs with the wrong format but allows absent ones', () => {
    expect(
      validateForEInvoicing(makeInvoice({ customerTrn: 'ABC' }), lines, company)
        .some((i) => i.field === 'invoice.customerTrn'),
    ).toBe(true);
    expect(
      validateForEInvoicing(makeInvoice({ customerTrn: null }), lines, company),
    ).toEqual([]);
  });
  ...
  it('catches totals that disagree with the lines', () => {
    ...
    const vatIssues = validateForEInvoicing(makeInvoice({ vatAmount: 500, total: 1800 }), lines, company);
    expect(vatIssues.some((i) => i.field === 'invoice.vatAmount')).toBe(true);
  });

  it('catches subtotal+VAT vs total drift and empty invoices', () => {
    expect(
      validateForEInvoicing(makeInvoice({ total: 2000 }), lines, company)
        .some((i) => i.field === 'invoice.total'),
    ).toBe(true);
    expect(
      validateForEInvoicing(makeInvoice({ subtotal: 0, vatAmount: 0, total: 0 }), [], company)
        .some((i) => i.field === 'lines'),
    ).toBe(true);
  });
});

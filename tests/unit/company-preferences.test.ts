...
describe("companyPreferencesSchema", () => {
  ...
  it("rejects malformed contactEmail but accepts empty string", () => {
    expect(companyPreferencesSchema.safeParse({ contactEmail: "not-an-email" }).success).toBe(
      false
    );
  ...
  it("rejects unknown emirate", () => {
    expect(companyPreferencesSchema.safeParse({ emirate: "tatooine" }).success).toBe(false);
  });

  it("rejects unknown date format", () => {
    expect(companyPreferencesSchema.safeParse({ dateFormat: "DD-MMM-YY" }).success).toBe(false);
  });
});

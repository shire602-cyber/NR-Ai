...
import {
  ...
  type BookkeeperInterventionInput,
} from "../../server/services/firm-clients.service";

function baseInterventionInput(
  overrides: Partial<BookkeeperInterventionInput> = {}
): BookkeeperInterventionInput {
  return {
    priority: "on_track",
    nextBestAction: "Ready for review",
    ...
    missingCustomerTrnCount: 0,
    unpostedReceiptCount: 0,
    unreconciledBankCount: 0,
    daysSinceActivity: 1,
    noOperatingDocs: false,
    discrepancy: 0,
    ...overrides,
  };
}
...
describe("firm-clients.service: import mapping", () => {
  ...
  describe("bookkeeper intervention scoring", () => {
    it("ignores filed VAT and CT deadlines when scoring intervention risk", () => {
      const result = buildBookkeeperIntervention(
        baseInterventionInput({
          vat: {
            filed: true,
            daysTilDue: -14,
            blockers: ["VAT return pending review"],
          },
          corporateTax: {
            filed: true,
            daysTilDue: 0,
            blockers: ["Missing corporate tax registration"],
          },
        })
      );
      ...
      expect(result.reasons).toEqual(["No active intervention signals"]);
      expect(result.deadlineLabel).toBe("No deadline pressure");
      expect(result.title).toBe("Ready for review");
      expect(result.ownerAction).toBe("Ready for review");
    });

    it("adds VAT deadline pressure and a VAT reason when unfiled VAT is due today", () => {
      const result = buildBookkeeperIntervention(
        baseInterventionInput({
          vat: {
            filed: false,
            daysTilDue: 0,
            blockers: ["Missing VAT TRN"],
          },
        })
      );
      ...
      expect(result.level).toBe("low");
      expect(result.reasons).toContain("VAT due today");
      expect(result.deadlineLabel).toBe("VAT due today");
      expect(result.title).toBe("VAT filing at risk");
      expect(result.ownerAction).toBe("Escalate VAT filing");
    });

    it("only reports CT pressure when VAT is filed and CT is unfiled due soon", () => {
      const result = buildBookkeeperIntervention(
        baseInterventionInput({
          vat: {
            filed: true,
      ...
      expect(result.level).toBe("low");
      expect(result.reasons).toEqual(["CT 30d left"]);
      expect(result.reasons).not.toEqual(expect.arrayContaining([expect.stringContaining("VAT")]));
      expect(result.deadlineLabel).toBe("CT 30d left");
      expect(result.title).toBe("Corporate tax at risk");
      expect(result.ownerAction).toBe("Lock CT preparation plan");
    });
  });
});

...
describe("renderTemplate", () => {
  ...
  it("does not recursively expand placeholder values that look like tokens", () => {
    ...
    expect(renderTemplate("{x}", { x: "{customerName}", customerName: "Acme" })).toBe(
      "{customerName}"
    );
  });
  ...
  it("substitutes Arabic placeholder values without mangling characters", () => {
    expect(renderTemplate("عميل: {customerName}", { customerName: "شركة الفجر" })).toBe(
      "عميل: شركة الفجر"
    );
  });
...
describe("contextForInvoice", () => {
  it("uses contact name when present", () => {
    ...
    const ctx = contextForInvoice(
      row,
      { id: "c", name: "Real Co.", phone: "+971501234567" },
      {
        senderName: "Muhasib",
...
describe("groupByClient", () => {
  function row(
    ...
    return buildAgingRow(
      inv({
        ...
        customerName,
        contactId,
        dueDate: new Date(today.getTime() - days * 86400000).toISOString(),
        total,
      }),
      [],
      today
  ...
  it("uses normalised name fallback regardless of case/whitespace", () => {
    const rows = [row("  Acme LLC ", null, 5, 100, "a"), row("acme llc", null, 7, 50, "b")];
    const groups = groupByClient(rows);
    expect(groups).toHaveLength(1);
    expect(groups[0].rows).toHaveLength(2);
  });
...
describe("renderGroupedMessage", () => {
  it("renders single-invoice without bullet list", () => {
    ...
    const out = renderGroupedMessage(
      groups[0],
      { body: "{customerName}: {invoiceNumber} {amount}", subject: null },
      {
        senderName: "Sender",
        paymentLink: "",
      }
  ...
  it("appends invoice list when multiple", () => {
    const rows = [
      buildAgingRow(
        inv({ id: "a", number: "INV-A", dueDate: "2026-04-01", total: 100, contactId: "c" }),
        [],
        today
      ),
      buildAgingRow(
        inv({ id: "b", number: "INV-B", dueDate: "2026-03-01", total: 200, contactId: "c" }),
        [],
        today
      ),
    ];
    ...
    const out = renderGroupedMessage(
      groups[0],
      { body: "{customerName}", subject: "S {amount}" },
      {
        senderName: "X",
        paymentLink: "",
      }

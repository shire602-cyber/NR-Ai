                  ...
                  {comms.map((c) => (
                    <TableRow key={c.id}>
                      ...
                      <TableCell className="max-w-xs">
                        <div className="truncate text-sm">{c.subject || c.body.slice(0, 60)}</div>
                        {c.recipientEmail && (
...
function ComposeTab() {
  ...
  const sendMutation = useMutation({
    mutationFn: (data: Record<string, string>) =>
      apiRequest(
        "POST",
        channel === "email" ? "/api/firm/comms/send-email" : "/api/firm/comms/send-whatsapp",
        data
      ),
...
function TemplatesTab() {
  ...
  const saveMutation = useMutation({
    mutationFn: (data: Partial<Template>) => apiRequest("POST", "/api/firm/comms/templates", data),
    onSuccess: () => {
                ...
                <TableBody>
                  {templates?.map((tmpl) => (
                    <TableRow key={tmpl.id}>
                      ...
                      <TableCell>
                        {templateTypeLabel[tmpl.templateType] ?? tmpl.templateType}
                      </TableCell>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setEditTemplate((p) => ({ ...p, channel: v as CommChannel }))
                  }
                >
                  <SelectTrigger>
                    <SelectValue />

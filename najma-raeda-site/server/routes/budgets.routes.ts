...
const monthAmount = z.union([z.string(), z.number()]).optional().nullable();
...
export function registerBudgetRoutes(app: Express) {
  ...
  app.get(
    "/api/companies/:companyId/budget-plans",
    authMiddleware,
    requireCustomer,
  ...
  app.post(
    ...
    authMiddleware,
    requireCustomer,
    validate({ body: budgetPlanCreateSchema }),
    asyncHandler(async (req: Request, res: Response) => {
      const { companyId } = req.params;
      const userId = (req as any).user.id;
  ...
  app.patch(
    ...
    authMiddleware,
    requireCustomer,
    validate({ body: budgetPlanUpdateSchema }),
    asyncHandler(async (req: Request, res: Response) => {
      const { id } = req.params;
      const userId = (req as any).user.id;
  ...
  app.post(
    ...
    authMiddleware,
    requireCustomer,
    validate({ body: budgetLineCreateSchema }),
    asyncHandler(async (req: Request, res: Response) => {
      const { id } = req.params;
      const userId = (req as any).user.id;
  ...
  app.patch(
    ...
    authMiddleware,
    requireCustomer,
    validate({ body: budgetLineUpdateSchema }),
    asyncHandler(async (req: Request, res: Response) => {
      const { id } = req.params;
      const userId = (req as any).user.id;

      const lineResult = await pool.query(
        `SELECT bl.*, bp.company_id FROM budget_lines bl JOIN budget_plans bp ON bl.budget_id = bp.id WHERE bl.id = $1`,
        [id]
      );

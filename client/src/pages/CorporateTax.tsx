...
import { exportToExcel, prepareCorporateTaxReturnForExport } from "@/lib/export";
import {
  FileCheck,
  Calculator,
  Loader2,
...
export default function CorporateTax() {
  ...
  const exportCorporateTax = async (
    taxReturn:
      | CorporateTaxReturn
      | (CalculationResult & { totalDeductions: number; status?: string; notes?: string })
  ) => {
    try {
      await exportToExcel(
        prepareCorporateTaxReturnForExport(taxReturn),
        `corporate-tax-working-paper-${format(
          new Date((taxReturn as any).taxPeriodStart ?? (taxReturn as any).periodStart),
          "yyyy-MM-dd"
        )}`
      );
      toast({
        title: "Excel Exported",
        description: "Corporate tax working paper has been downloaded.",
      });
    } catch (error: any) {
      toast({
        variant: "destructive",
        title: "Export Failed",
        description: error?.message || "Failed to export corporate tax workbook",
      });
    }
  };
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              UAE Corporate Tax working paper - 9% on taxable income above AED 375,000
            </p>
          </div>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Calculate a reviewable corporate tax working paper from posted journal entries
          </CardDescription>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Saved corporate tax working papers and their filing status
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoadingReturns ? (
                        ...
                        <TableCell className="text-right">
                          <div className="flex items-center justify-end gap-1">
                            ...
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => void exportCorporateTax(taxReturn)}
                              title="Download Excel working paper"
                            >
                            ...
                            {taxReturn.status === "draft" && (
                              <>
                                <Button
                                  ...
                                  title="Mark as externally filed"
                                >
      ...
      <Dialog open={viewDialogOpen} onOpenChange={setViewDialogOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button onClick={() => viewReturn && void exportCorporateTax(viewReturn)}>
              <FileSpreadsheet className="w-4 h-4 mr-2" />
              Download Excel
            </Button>
          </DialogFooter>

...
import { exportToExcel, prepareVAT201ReturnForExport } from "@/lib/export";
...
export default function VATFiling() {
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "Your VAT return is saved for review before external filing.",
      });
      setEditDialogOpen(false);
  ...
  const handleExportExcel = async (vatReturn: VATReturn) => {
    try {
      await exportToExcel(
        prepareVAT201ReturnForExport(vatReturn),
        `VAT201-working-paper-${format(parseISO(vatReturn.periodStart), "yyyy-MM")}`
      );
      toast({
        title: "Excel Exported",
        description: "VAT 201 working paper has been downloaded.",
      });
    } catch (error: any) {
      toast({
        variant: "destructive",
        title: "Export Failed",
        description: error?.message || "Failed to export VAT 201 workbook",
      });
    }
  };
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            {locale === "ar"
              ? "إنشاء أوراق عمل ضريبة القيمة المضافة 201 ومراجعتها قبل التقديم"
              : "Generate VAT 201 working papers for review before filing"}
          </p>
        </div>
              ...
              <div>
                ...
                <p className="text-sm text-amber-700 dark:text-amber-300">
                  {locale === "ar"
                    ...
                    : "Please add your TRN in Company Profile before preparing VAT filing packs."}
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <CardHeader>
          ...
          <CardDescription>
            {locale === "ar"
              ...
              : "All your VAT return working papers, submissions, and drafts"}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoadingReturns ? (
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <Button
                            ...
                            data-testid={`button-export-excel-${vatReturn.id}`}
        ...
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
          ...
          <DialogFooter className="gap-2">
            ...
            <Button onClick={handleSubmitReturn} disabled={submitMutation.isPending}>
              ...
              {locale === "ar" ? "تقديم للمراجعة" : "Submit for Review"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

...
export function registerDocumentVersionRoutes(app: Express) {
  ...
  app.get(
    "/api/companies/:companyId/document-versions/:documentType/:documentId",
    authMiddleware,
    requireCustomer,
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (!validTypes.includes(documentType)) {
        return res
          .status(400)
          .json({ message: `Invalid document type. Must be one of: ${validTypes.join(", ")}` });
      }
  ...
  app.post(
    "/api/companies/:companyId/document-versions",
    authMiddleware,
    requireCustomer,
    asyncHandler(async (req: Request, res: Response) => {
      const userId = (req as any).user.id;
      ...
      const existingCount = await storage.getDocumentVersionCount(
        companyId,
        documentType,
        documentId
      );

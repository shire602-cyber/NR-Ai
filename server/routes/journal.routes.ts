  ...
  app.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (totalDebit === 0 || totalCredit === 0) {
        return res
          .status(400)
          .json({ message: "Entry must have at least one debit and one credit" });
      }
  ...
  app.put(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (entry.status === "posted") {
        return res.status(400).json({
          message:
            "Posted journal entries cannot be edited. Use reversal to correct posted entries.",
          code: "ENTRY_POSTED",
        });
      }
      ...
      res.json({
        ...
        message: "Draft entry updated successfully",
      });
    })
  );
  ...
  app.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (entry.status !== "draft") {
        ...
        return res
          .status(400)
          .json({ message: errorMessage, code: `ENTRY_${entry.status.toUpperCase()}` });
      }
  ...
  app.delete(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      assertRetentionExpired(
        entry as { createdAt: Date | string; retentionExpiresAt?: Date | string | null },
        "Journal entry"
      );

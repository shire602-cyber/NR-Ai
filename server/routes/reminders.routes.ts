...
export function registerReminderRoutes(app: Express) {
  ...
  app.patch(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      const validated = reminderSettingSchema.partial().parse(req.body);
      const setting = await storage.updateReminderSetting(id, validated);
      res.json(setting);
    })
  );
  ...
  // Get reminder logs
  app.get(
    "/api/companies/:companyId/reminder-logs",
    authMiddleware,
    asyncHandler(async (req: Request, res: Response) => {
      const { companyId } = req.params;
      const userId = (req as any).user?.id;
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (company?.trnVatNumber) {
        ...
        deadlines.push({
          ...
          priority:
            Math.ceil((nextDeadline.getTime() - today.getTime()) / (1000 * 60 * 60 * 24)) <= 7
              ? "high"
              : "normal",
          actionUrl: "/reports",
        });

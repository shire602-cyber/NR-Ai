...
export function registerAIRoutes(app: Express) {
  ...
  function getOpenAI(): OpenAI {
    if (!openai)
      throw Object.assign(new Error("AI service not configured — set OPENAI_API_KEY"), {
        status: 503,
      });
    return openai;
  }
    ...
    asyncHandler(async (req: Request, res: Response) => {
      try {
        ...
        try {
          ...
          log.warn(
            { err: err?.message || err },
            "Internal classifier failed — falling through to OpenAI"
          );
        }
        ...
        if (internal && internal.method !== "openai" && internal.confidence >= 0.8) {
          ...
          const suggested = internal.accountId
            ...
            : expenseAccounts.find((a) =>
                a.nameEn.toLowerCase().includes(internal.category.toLowerCase())
              );
          if (suggested) {
            return res.json({
        ...
        const accountList = expenseAccounts
          .map((acc) => `${acc.nameEn}${acc.nameAr ? ` (${acc.nameAr})` : ""}`)
          .join("\n");
    ...
    asyncHandler(async (req: Request, res: Response) => {
      try {
        ...
        const accountList = allAccounts
          .map((acc) => `${acc.nameEn} (${acc.type})${acc.nameAr ? ` - ${acc.nameAr}` : ""}`)
          .join("\n");
        ...
        for (let i = 0; i < transactions.length; i++) {
          ...
          try {
            const r = await classifyOcrReceipt(
              companyId,
              {
            ...
            if (r.method !== "openai" && r.confidence >= 0.8) {
              const matched = r.accountId
                ...
                : expenseAccounts.find((a) =>
                    a.nameEn.toLowerCase().includes(r.category.toLowerCase())
                  );
              internalResults[i] = {
                category: r.category,
            ...
            log.warn(
              { err: err?.message, idx: i },
              "Internal classifier failed for batch item — falling back to OpenAI"
            );
            remaining.push({ originalIndex: i, t });
          }
        }
        ...
        const previousClassifications =
          await storage.getTransactionClassificationsByCompanyId(companyId);
        ...
        const transactionList = remaining
          .map(
            (row, i) =>
...
${
  learningContext
    ? `Previous learned patterns (user-confirmed):
${learningContext}`
    : ""
}
  ...
  "classifications": [
    {
      ...
      "flags": ["unusual_amount", "duplicate_risk"] // optional warnings
          ...
          for (const classification of aiResponse.classifications || []) {
            ...
            const remainingIdx =
              typeof classification.index === "number" ? classification.index : -1;
            const row =
              remainingIdx >= 0 && remainingIdx < remaining.length ? remaining[remainingIdx] : null;
            if (!row) continue;
            const t = row.t;
            allClassifications.push({
              ...classification,
...
Respond with JSON:
{
  ...
  "summary": {
    ...
    "unusualTransactions": 2
        ...
        for (const anomaly of aiResponse.anomalies || []) {
          const safeRelatedId = await validateBelongsToCompany(
            anomaly.entityType,
            anomaly.entityId
          );
          // duplicateOfId must reference the same entity type as the anomaly
          // (a "duplicate invoice" alert points at another invoice).
          const safeDuplicateId = await validateBelongsToCompany(
            anomaly.entityType,
            anomaly.duplicateOfId
          );
  ...
  // Get Anomaly Alerts
  app.get(
    "/api/companies/:companyId/anomaly-alerts",
    authMiddleware,
    requireCustomer,
    asyncHandler(async (req: Request, res: Response) => {
      try {
...
${JSON.stringify(ledgerData, null, 2)}`,
  ...
  app.get(
    "/api/companies/:companyId/bank-transactions",
    authMiddleware,
    requireCustomer,
    asyncHandler(async (req: Request, res: Response) => {
      try {
...
Respond with JSON:
{
  ...
  app.get(
    "/api/companies/:companyId/forecasts",
    authMiddleware,
      ...
      try {
        ...
        if (typeof userSelectedAccountId === "string") {
          ...
          if (!account) {
            return res
              .status(403)
              .json({ message: "userSelectedAccountId does not belong to this company" });
          }
        }
  ...
  const autopilotProcessSchema = z.object({
    ...
    ocr: z.object({
      ...
      date: z
        ...
        .optional(),
      lineItems: z
        .array(z.object({ description: z.string().max(500).optional() }).passthrough())
        .max(200)
        .optional()
        .default([]),
        ...
        const financialSummary = {
          ...
          expensesSummary: {
            ...
            totalAmount: receipts.reduce(
              (sum, r) => sum + Number(r.amount || 0) + Number(r.vatAmount || 0),
              0
            ),
          },
...
Company: ${company.name}`;
        ...
        const assistantMessage =
          response.choices[0]?.message?.content || "I could not process your request.";
...
${askGuidanceBlock}`;
        ...
        if (validated.stream) {
          ...
          try {
            ...
            const errorMessage =
              streamError.message || "An error occurred while streaming the response";
            res.write(
              `data: ${JSON.stringify({
                error: true,
      ...
      try {
        ...
        if (query) {
          ...
          accounts = accounts.filter(
            (a) =>
              a.nameEn.toLowerCase().includes(q) || (a.nameAr && a.nameAr.toLowerCase().includes(q))
          );
        }
        ...
        res.json(
          accounts.slice(0, Number(limit)).map((a) => ({
            id: a.id,
            nameEn: a.nameEn,
            nameAr: a.nameAr,
            type: a.type,
      ...
      try {
        ...
        if (query) {
          ...
          customers = customers.filter(
            (c) => c.name.toLowerCase().includes(q) || c.trn?.toLowerCase().includes(q)
          );
        }
        ...
        res.json(
          customers.slice(0, Number(limit)).map((c) => ({
            name: c.name,
            trn: c.trn,
    ...
    asyncHandler(async (req: Request, res: Response) => {
      try {
        ...
        const merchantMap = new Map<
          string,
          { name: string; category: string | null; count: number; lastAmount: number }
        >();
        ...
        res.json(
          merchants.slice(0, Number(limit)).map((m) => ({
            name: m.name,
            category: m.category,
  ...
  app.get(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      try {
        ...
        res.json(
          results.slice(0, Number(limit)).map((d) => ({
      ...
      try {
        ...
        if (fieldType === "account" && context.merchant) {
          ...
          const merchantReceipts = receipts.filter(
            (r) => r.merchant?.toLowerCase() === context.merchant.toLowerCase() && r.journalEntryId
          );
...
Respond with just the account name, nothing else.`;
            ...
            const matchedAccount = expenseAccounts.find(
              (a) => a.nameEn.toLowerCase() === suggestedName?.toLowerCase()
            );

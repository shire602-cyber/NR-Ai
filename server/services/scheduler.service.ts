...
import { scanDueReportPacks } from "./report-pack-schedules.service";
...
export function initScheduler() {
  ...
  // Run daily at 07:00 UTC: prepare due persona report packs.
  cron.schedule("0 7 * * *", async () => {
    try {
      log.info("Running daily report pack preparation scan...");
      const result = await scanDueReportPacks();
      log.info(result, "Daily report pack preparation scan complete");
    } catch (err) {
      log.error({ err }, "Scheduler error during report pack preparation scan");
    }
  });
  ...
  log.info(
    "Scheduler initialized — payment scans hourly, GL scans every 30min, recurring invoices daily at 06:00 UTC, report packs daily at 07:00 UTC, auth-token sweep hourly"
  );
}

...
function addTitleBlock(
  ...
  const meta: string[][] = [
    ...
    ['Tax period', `${isoDate(ctReturn.taxPeriodStart)} → ${isoDate(ctReturn.taxPeriodEnd)}`, 'Status', String(ctReturn.status)],
...
function addWorkpaperSheet(
  ...
  for (const section of ['revenue', 'expense'] as const) {
    ...
    sectionHeader.getCell(1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: SECTION_COLOR } };

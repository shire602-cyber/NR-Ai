...
export interface ManualAutoPostResult {
  autoPosted: boolean;
  journalEntryId: string | null;
  /** Machine-readable reason: posted, eligible, or why it was skipped. */
  reason: string;
}

/**
 * Auto-post an already-saved, human-reviewed manual receipt. The receipt row
 * already exists with the user's reviewed data; this only decides whether to
 * post and, if eligible, creates the balanced journal entry and links it back.
 * Safe by default: returns early unless the company explicitly enabled autopilot.
 */
export async function autoPostManualReceipt(args: {
  ...
  // Cheap guards first. Most companies have autopilot off, so avoid the
  // classification round-trip when the save is obviously ineligible.
  ...
  // Re-classify server-side so the auto-post decision is authoritative and
  // never depends on client-supplied confidence or method data.
  const model = await getModel(companyId);
  const classification = await classifyReceipt({

...
export class DatabaseStorage implements IStorage {
  ...
  async getActiveWebhookEndpointsForEvent(
    companyId: string,
    event: string
  ...
  async getWebhookDeliveriesByEndpointId(
    endpointId: string,
    limit = 50
  ...
  async updateInvoiceTemplate(
    id: string,
    data: Partial<InsertInvoiceTemplate>
  ...
  async updateReconciliationRule(
    id: string,
    data: Partial<InsertReconciliationRule>
  ): Promise<ReconciliationRule | undefined> {
    const [rule] = await db
  ...
  async getPurchaseOrderLinesByPurchaseOrderId(
  ...
  async recordInvoicePayment(input: {
    ...
    return await db.transaction(async (tx: typeof db) => {
      ...
      const lockResult: any = await tx.execute(sql`
        SELECT id, company_id, currency, total, status, number, exchange_rate
        FROM invoices

...
- **Double-reversal bug** — reversing a journal entry both voided the original *and* posted the offset, corrupting the GL by −1×. Fixed to standard treatment (original stays posted, pair nets to zero).
...
3. **Stripe live checkout** requires `STRIPE_SECRET_KEY` and price IDs in Railway *if/when* paid billing is switched on (`BILLING_ENFORCEMENT=true`). Billing is intentionally not enforced today.
...
*Prepared by automated end-to-end verification, 2026-06-13.*
